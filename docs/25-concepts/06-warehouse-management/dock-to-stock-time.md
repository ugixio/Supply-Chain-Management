...
- **Inputs:** two timestamps on a common clock (implementation takes hours-since-midnight
  floats).
- **Outputs:** the elapsed time, in hours and in minutes.
- **Project-chosen input:** the level that counts as acceptable. It depends on the goods and the
  service promise, and nothing external fixes it — a two-hour bar is one warehouse's target, not a
  standard. This node defines how the time is *measured*; the comparison is the project's.
...
- Goods needing inspection or a QA hold legitimately take longer than free-flow ambient receipt —
  segment the measure by inspection class before acting on it, whatever level is set.

## Worked example

Arrive 08:30 (8.5), putaway complete 10.25 → `DTS = 1.75 h = 105 min`.
