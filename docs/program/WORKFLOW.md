...
- 🟦 **X13b · The `security/` axis — landed 2026-08-04 (ADR-0055).** Owner authorized both X13 answers;
  this is the additive one and it went first. **SEC-R1..R3** now hold ADR-0054's three statements, and
  `knowledge-architecture.md` §7 **cites** them instead of carrying their text — which corrects a §4
  violation committed one change earlier under exactly the pressure X13 names: there was **nowhere to
  allocate the ID**, so the rule was written as prose. The axis was pre-declared as a candidate needing
  owner authorization plus a cited need; both conditions recorded, same basis as `measurement/`
  (ADR-0039). The rule file states **per rule** whether a gate checks it, and two of three are not
  checkable — a family that lets a reader assume enforcement it does not have is worse than one that
  says where enforcement stops. Scope held to the estate as an *input to an agent*: `authZ, secrets,
  audit` stays unrecorded and arrives with M4.
- 🟨 **X13a · G9's `rule` budget is a ratchet, and two families are at it (improvement #49).** Found by
  ...
  **Owner authorized both, 2026-08-04.** (b) landed as X13b above. **(a) is what remains here** — and it
  is the one with a real trade-off to hold in view: today a reader checking whether a retired ID is
  still law opens its own `rule.md` and finds the retirement note beside the live rules, and afterwards they open a
  register. That locality is the reason CLAUDE.md says *retired IDs stay listed as retired so old
  citations still resolve*. The move is still right — 4,459 words is not rationale, it is a filing
  cabinet inside a budgeted file — but the register must carry the **reason** for each retirement, not
  only the ID, or the move trades a budget for a worse answer to the question people actually ask.
  Raising the number is not on the list — the estate has refused that five times.
